...
n=len(sys.argv)
if n!=2:
    print("Usage: python3 machine_translation1.py <input_file>")
    exit()
print(sys.argv)
model_path = sys.argv[1]
# ../saved_pkl/MT-1
# ../saved_pkl/MT-2
# model_path="/content/drive/MyDrive/colab/MT-1"
...
train_data,valid_data,test_data=TabularDataset.splits(path="./",train="train.csv",validation="dev.csv",test="test.csv",format="csv",fields=[('source',en_field),('target',fr_field)],skip_header=True)
...
encoder=Encoder(len(en_field.vocab.itos),embedding_count,lstm_cell_count,number_layers,dropout_value).to(device)
decoder=Decoder(len(fr_field.vocab.itos),embedding_count,lstm_cell_count,dropout_value,len(fr_field.vocab.itos)).to(device)
model=Seq2Seq(encoder,decoder).to(device)
optimizer=optim.Adam(model.parameters(),lr=1e-3)
criterion=nn.CrossEntropyLoss(ignore_index=fr_field.vocab.stoi["<pad>"])
# optimizer=optim.RMSprop(model.parameters(),lr=1e-3)

# model.load_state_dict(torch.load("/content/drive/MyDrive/Colab Notebooks/sub/MT-AAZ_0")['model'])
